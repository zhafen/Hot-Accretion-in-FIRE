...
def colors_table( age_in_Gyr, metallicity_in_solar_units, 
    colors_sps_path = '/home1/03057/zhafen/papers/Hot-Accretion-in-FIRE/data',
    ...
    #if you make it this far, you're actually doing the calculation
    if (QUIET==0): 
        print('Calculating M/L in '+str(b[band])+' ('+str(band)+','+str(band_standardordering)+')')

    froot = colors_sps_path+'/colors_sps/'; # directory in which the data binaries are stored
...
def get_attenuated_stellar_luminosities( BAND_IDS, \
    ...
    Nstar=checklen(stellar_age);

    ## alright now we're ready to get the (intrinsic) stellar luminosities
    nband=checklen(BAND_IDS); lums=np.zeros([nband,Nstar]); 
