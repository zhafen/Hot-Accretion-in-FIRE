...
Try adding radial velocity panel.
Move Rvc(R) label to the left.
Move aligned label down.
Address the increase in jz in panel two.Torquing of the disk? Collisional interaction? Evidence in the form of hot gas rotating slower?
Try removing shaded region   for Rvc, because we primarily just care about the average angular support.
Scatter in jz goes below zero, try showing that.
Try showing for the third panel ( jvec - jz )/j, which we expect to go to zero, and shows the coherence and only the coherence pretty well.
...
Reproduce this analysis for simulations with cold mode accretion so we have an idea of what could be different.
...
% Tjitske comments about second and third panel
% \textbf{
% You may be right. What I meant was that it seems from panel 3 that there is a lot of change of angular momentum direction: the allowed range is -1 - +1 and the 1-sigma distribution spans close to that entire range. But both j_z and |j| don't have very large scatter and are pretty constant. So what is going on? especially at the low j_z/|j| end? I expect those particles have maybe very low |j| anyway so small changes in j_z can mean big changes in j_z/|j|?
% My sense is that people tend to interpret a distribution like in the 3rd panel as a distribution with not very aligned angular momentum and a lot of evolution, but the 2nd panels show that this is actually not the case. So I'm trying to understand what the 3rd panel really means.
% Looking at the 2d histograms or 3d distributions may help getting more intuition?
% }
