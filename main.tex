...
The dominance of this ``rotating cooling flow'' accretion is strongly correlated with the fraction of thin disk stars in the central galaxy across a sample of \Nsample~galaxies spanning a halo mass range of $10^{10.5} M_\odot \lesssim M_{\rm h} \lesssim 10^{12} M_\odot$.
We show that this correlation traces to angular momentum exchange between hot accreting gas;
this allows accretion to become coherent and therefore quickly collapse into a thin gaseous disk when cooled to $T \approx 10^4 K$.
Notably, central galaxies with a thick disk or irregular morphology do not accrete via rotating cooling flows, but rather are supplied by cold accretion that cools far beyond the galaxy edge.
...
Despite this understanding, a detailed accounting of the origin of disk formation in a cosmological context is incomplete --- the detailed properties of disks formed in cosmological simulations cannot be explained by a tight coupling between dark matter spin and baryon spin~\citep[e.g.][]{GK18}.  
...
% Cold mode
In cold mode accretion, cool, $T \sim 10^4$ K cosmological filaments travel into the galaxy from the IGM~\cite[e.g.][]{Keres2005, Dekel2006, Keres2009, Martin2019a}, often with by embedded satellite galaxies~\citep[e.g.][]{Hafen2019, Esmerian2020}.
...
One subset of hot-mode accretion is instability-driven accretion, wherein gas precipitates out of the hot halo due to thermal instabilities, forming cool clouds which lose buoyancy and accrete onto the galaxy~\citep[e.g.][]{Maller2004, Mccourt2012, Voit2015, Armillotta2016, Gronke2019a, Voit2021}.  
% Quiet accretion
...
In this paper we demonstrate that cooling flows that accrete onto the galaxy while spinning are the primary mode of gas accretion onto disky Milky Way-mass galaxies at $z \sim 0$ in the FIRE simulations. 
...
\hline
Name  &  $f_{\rm thin\,disk,\,young}$  &  $M_{\rm vir}$  &  $M_\star$  &  $R_{\textrm{vir}}$  &  $\Delta f_{\rm aligned}$  &  Physics  &  Reference  \\
  &   &  $M_\odot$  & $M_\odot$  &  kpc  &  &  &  \\
 \hline
...
\textbf{Update columns with additional diskiness estimates.}
\end{flushleft}
...
Figure~\ref{f: stars} shows edge-on mock Hubble images (with no dust attenuation) for four of our simulated galaxies.
...
For a given galaxy we select all resolution elements (particles) found in the central galaxy at $z=0$ and in the CGM 1 Gyr prior.
The galaxy is defined as all gas and stars inside the galaxy radius $R_{\rm gal}$, with an additional density cut of $n_{\rm H} = 0.13$ cm$^{-3}$ for gas.
...
Of the particles targeted to be tracked $<2\%$ are particles that accumulate over their history at least twice their mass' worth in deposited mass from stellar feedback, resulting in them being split into two particles.
...
As done in our previous particle-tracking analyses~\citep{Hafen2019, Hafen2020}, we discard these particles, which are not expected to affect our results due to their small contribution.
...
We focus our analysis on three MW-mass galaxies (\texttt{m12i}, \texttt{m12b}, and \texttt{m12c}) and contrast the process with accretion onto a $M_\star \sim 10^9 M_\odot$ galaxy, \texttt{m11q}.
...
\begin{figure*}
    ...
    Hot mode ($T>10^5$ K) gas accretion dominates the mass inflow onto thin-disk galaxies at $r \gtrsim R_{\rm gal} \sim 10-20$ kpc.
...
This also allows us to distinguish a hot inflow from cool clouds formed via instabilities, i.e. instability-driven accretion.
...
The cold inflow rate increases sharply within $R_{\rm gal}$, reflective of cooling at the galaxy-halo interface (next section).
...
Figure~\ref{f: R1e5K} shows the distribution of $\Rcon$ for our representative subsample of galaxies.
All particles with $\Rcon>80$ kpc were placed in the $R=80$ kpc bin.
...
\textit{The majority of the gas cools just outside the galaxy}, with a median of $\Rcon \approx 0.05 R_{\rm vir}$ and $\lesssim 10\%$ of particles cooling beyond $\sim 40$ kpc.
...
\texttt{m11q} has a larger $R_{\rm gal} / R_{\rm vir}$ than the higher mass halos, so the peak of cooling happens at a similar factor of $R_{\rm gal}$ as in the MW-mass halos.
...
\begin{figure*}
    ...
    For the MW-mass disk galaxies accreting gas is quasi-spherical prior to cooling.
    After cooling the gas has a disk-like configuration, i.e. accreting gas collapses quickly into a disk as it cools. 
...
The plot shows that gas accreting onto $\sim L^\star$ FIRE galaxies transitions from being distributed quasi-spherically prior to cooling to being distributed in the galaxy plane after cooling, over $\lesssim 400$ Myr.
...
\begin{figure*}
    ...
    \caption{
    \textbf{Left:}
    Fraction of accreting mass strongly aligned with the disk ($f_{\rm aligned} \equiv M(\mid z/R \mid < 0.1)/M$) over 200 Myr before and after cooling (red and blue respectively) for our sample of \Nsample~analyzed halos.
    ...
    \textbf{Right:}
    Change in aligned mass fraction over $\pm 200$ Myr before/after accreting gas cools below $T = 10^5$ K, colored by virial mass.
...
We relate alignment to thin disk formation via the fraction of stars formed between $z=0$ and a Gyr prior that reside in the thin disk at $z=0$ ($f_{\rm thin\,disk}(z=0,$ age$<1$ Gyr), or $f_{\rm thin\,disk,\,young}$/$f_{\rm thin\,disk}$ when abbreviated).
...
In halos with $f_{\rm thin\,disk} > 0.5$ the alignment of the accreted gas sharply increases by 200 Myr after $\tcon$---in most cases $\gtrsim 50\%$ of mass collapses to the $10\%$ of the volume that contains the majority of the thin disk stars.
On the other hand, the majority of the accretion onto galaxies with $f_{\rm thin\,disk} \approx 0$ does not align within $\tcon+200$ Myr, retaining $f_{\rm aligned}< 0.2$.
In these halos cooling of accreted gas is not associated with circularization.


% Change in aligned mass fraction
The link between circularized cooling flow accretion and thin disk formation is evident in the right panel of Figure~\ref{f: prevalence}, which shows $\Delta f_{\rm aligned}$, the change over 400 Myrs before/after cooling in the fraction of accreting mass that is aligned.
Because gas that accretes as part of a rotating cooling flow changes alignment as it accretes, this metric scales with the fraction of accretion that accretes as a rotating cooling flow.
...
% Figure coherence descrption
Figure~\ref{f: coherence} shows the evolution of angular momentum coherence by displaying a series of mass distributions of $j_z / \vert \vec j \vert$.
...
We now go through these figures in detail.

\subsubsection{Accretion becomes coherent in the halo}
\label{s: mechanics -- coherence}

% Description
Figure~\ref{f: coherence} is the angular momentum analog to Figure~\ref{f: theta vs t}.
The displayed distributions center on 61 times evenly spaced over $- 1\, {\rm Gyr} < t_{\rm bin} - \tcon < 0.5\, {\rm Gyr}$.
Each distribution includes $j_z / \vert \vec j \vert$ values for all particles with $t - \tcon$ within $\pm$30 Myr of $t_{\rm bin}$.
...
% Overall evolution
Over the course of 1.5 Gyr, accretion onto thin-disk galaxies transitions from decoherent, gently-sloping distributions similar to the isotropic distribution, to highly coherent distributions with $j_z/\vert \vec j \vert > 0.9$ for $\gtrsim 90\%$ of accretion.
...
\subsubsection{Hot, inflowing, extended to cooled, stalled, collapsed}
...
The median $j_z$ is equal to $\approx 2500$ kpc km s$^{-1}$, comparable to the average specific angular momentum of $0.035\lambda \vvir \Rvir=\mathbf{xx}\lambda$ expected in dark matter halos due to tidal torques (\textbf{citation needed}, where $\lambda$ is the spin parameter and $\vvir$ is the virial velocity).
...
\begin{itemize}
    ...
    As outlined in Figure~\ref{f: before and after}, cooling occurs when the gas becomes rotationally supported --- this causes a pileup that increases density and therefore radiative cooling, while at the same time halting compressive heating driven by inward movement.
\end{itemize}
