...
Across a sample of \Nsample~galaxies spanning $10^{11} M_\odot < M_{\rm h} < 10^{13} M_\odot$ the fraction of thin disk stars formed is tightly correlated with the fraction of mass accreted as a rotating cooling flow.
\end{abstract}
...
% FIRE simulations
Our analysis makes extensive use of hydrodynamical cosmological zoom-in simulations produced as part of the FIRE project\footnote{\url{https://fire.northwestern.edu/}}~\citep{Hopkins2014}.
...
\subsubsection{Gas flow is hot through the CGM}
...
Figure~\ref{f: Mdot} shows the mass inflow rate of different gas phases at a given radius in the same representative sample as Figure~\ref{f: stars}.
...
\subsubsection{Gas cools at the galaxy-halo interface}
...
\begin{figure*}
    ...
    The absence of accretion with $\Rcon \gtrsim 0.2 R_{\rm vir}$ is evidence that gas accretion onto MW-mass FIRE galaxies is not the result of precipitation at large radii.
...
Note that gas that cools inside $R_{\rm gal}$ without is still cooling outside the galaxy because it is under-dense relative to the galaxy ($n_{\rm H} < 0.13$ cm$^{-3}$).

\subsubsection{Accreting gas collapses directly into a disk}
...
\begin{figure*}
    ...
    \caption{
    Vertical distribution of accreting gas over 150 Myr before/after cooling.
    Angle is relative to the stellar disk, with $\cos\theta = z/R = 0$ for gas along the disk plane.
...
\begin{figure*}
    ...
    \caption{
    \textbf{Left:}
    Fraction of accreting mass strongly aligned with the disk (Aligned Accretion $\equiv M(\mid z/R \mid < 0.1)/M$) over 150 Myr before and after cooling (red and blue respectively) for our sample of \Nsample~analyzed halos.
    ...
    \textbf{Right:}
    Change in aligned mass over $\pm 150$ Myr before/after accreting gas drops below $T = 10^5$ K, defined to scale with the fraction of mass accreted as a rotating cooling flow.
    $\Delta($Aligned Accretion) strongly tracks thin disk fraction.
    }
    \label{f: prevalence}
\end{figure*}
...
\begin{figure*}
    ...
    Both thin disk fraction and $\Delta$(Aligned Accretion)$\sim 0$ for $M_{\rm vir} < 10^{12} M_\odot$ or $M_\star < M_{\star, {\rm MW}}$, and span a wide range for $\sim L_\star$ galaxies.
...
% Figure description
Figure~\ref{f: prevalence} shows for a sample of halos simulated to $z=0$  which halos have accreting gas that aligns with the galaxy as it cools, a property of rotating cooling flow accretion.
We parametrize the alignment of accreting gas as the fraction of mass with $\vert z/R \vert < 0.1$ (noted on Figure~\ref{f: theta vs t}), a range that contains $\approx 60\%$ of stars in galaxy's thin disk.
...
For most halos the accreting gas is largely unaligned prior to cooling, with aligned mass fractions Aligned Accretion $\equiv M(\vert z/R \vert  <0.1)/M\sim 0.1 - 0.2$.
For reference, $0.1$ is the value for an isotropic distribution, and $0.6$ is the value for thin disk stars in \texttt{m12i\_md}.
...
The link between circularized cooling flow accretion and thin disk formation is evident in the right panel of Figure~\ref{f: prevalence}, which shows the  change over 300 Myrs before/after cooling in the fraction of accreting mass that is aligned, $\Delta$(Aligned Accretion).
