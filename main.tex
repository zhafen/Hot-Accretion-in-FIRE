...
\begin{figure*}
    ...
    The Milky Way-mass galaxies have thin disk morphologies (top, bottom-left), while the lower mass galaxy has an irregular morphology (bottom-right).
    }
    \label{f: stars}
\end{figure*}
...
In this latter scenario, compression heating of the hot phase due to the inflow roughly balances radiative losses, so the hot gas stays at an approximately constant temperature.
...
This goes beyond 1D steady-state solutions for rotating cooling flows developed in classic ICM studies~\citep[e.g.][]{Cowie1980}, and extends the idealised 3D simulations of rotating cooling flows in \cite{Stern2020} to a more realistic cosmological setting. 
...
m11q  &  0.0066  &  $1.5\times10^{11}$  &  $7.4\times10^{8}$  &  138  &  0.0045  &  \checkmark  &  \cite{Hopkins2018}    \\ %\hline
...
The \texttt{m12m} galaxy has a sizable bar, which 
% forms secularly at late times~\citep{Debattista2019} and 
drives the thin-disk fraction lower by our adopted definition (marked by $^*$).
...
For each galaxy we use the particle tracking method described in \cite{Hafen2019, Hafen2020}, which in turn is built on insight from previous particle-tracking applied to the FIRE simulations~\citep{Angles-Alcazar2017}.
...
% Sample validity
In Appendix~\ref{s: appendix-sample validation} we show that the accretion histories of selected particles are representative of the accretion histories of all stars formed within $1$ Gyr of $z=0$, and
...
% In Appendix~\ref{s: appendix-sample validation} We also show there 
that around thin disk galaxies our selected particles are drawn from the angular momentum distribution of the CGM without bias.
...
The trajectories were visualized using the Firefly visualization software~\citep{Geller2018}.\footnote{
A 3D version of these trajectories is available online at \url{zhafen.github.io/rotating-cooling-flows}. The Firefly homepage is at \url{alexbgurvi.ch/Firefly}.}
...
\textbf{A:}
Temperature.
The inflow is predominantly hot ($\gtrsim 10^5$ K) prior to cooling, with a median temperature approaching $\Tvir\approx10^6$ K.
...
\begin{figure*}
    ...
    In thin disk galaxies (top and bottom-left panels) the geometry of accreting gas evolves significantly at the time of cooling, from a quasi-spherical distribution prior to cooling (red curves) to a disk-like configuration after cooling (blue curves). 
...
% and are further discussed in appendix~\ref{s: appendix-individual}.
...
% is of special note:
% gas accretes onto the halo as a rotating cooling flow, but subsequent evolution in the ISM and/or stellar population produces 
...
In the CGM where the accreting gas is hot, compression heating offsets radiative cooling. This yields the mostly flat temperature profile seen in Fig.~\ref{f: before and after A}, as expected in a cooling flow.
}
...
The shaded region in panel (A) shows the 16th--84th percentiles in $j_z/\vert\vec j\vert$, the ratio of angular momentum in the $z$ direction to the magnitude of total angular momentum, for individual particles as a function of $t-\tcools$. The solid line plots $(\Sigma \vec{j})_z/\vert\Sigma \vec{j}\vert$, which is the same ratio for the vector sum of all particles at a given $t-\tcools$. Note that this latter ratio is not identical to unity because the $z$ direction is with respect to the central galaxy rather than the gas itself. 
...
On the other hand, $(\Sigma \vec{j})_z/\vert\Sigma \vec{j}\vert$ is nearly constant with time prior to accretion. 
...
% for optically thin photoionized gas, which we take from \cite{Wiersma2009a}.
...
\begin{figure*}
    ...
    Over the course of $1$ Gyr prior to cooling the angular momentum distribution narrows significantly (i.e. becomes more coherent), while the direction of the net vector changes only slightly.
    }
...
\begin{figure}
    ...
    The strong correlation between the two properties suggests that achieving angular momentum coherence in the CGM is conducive to the formation of thin disks.
    % Galaxies with large thin disk fractions are fueled by accretion where half the mass becomes spin-aligned prior to entering the galaxy, in contrast with galaxies with small thin disk fractions are fueled by accretion that undergoes a milder increase in coherence prior to accretion. 
    }
...
This is due to the decrease in angular momentum dispersion prior to cooling (Figs.~\ref{f: coherence -- on sky}--\ref{f: coherence}, and panels A--B in Fig.~\ref{f: before and after B}) and due to the deceleration of the flow prior to accretion (panel D in Fig.~\ref{f: before and after A}).
...
Alternatively, thin disks could be rarer at high redshift due to evolving gas fractions, since in equilibrium disk models the thickness of the disk scales with  \citep{Thompson2005, Faucher-Giguere2013, Krumholz2018}.
...
% Precipitation
Our results also differ from precipitation models in that the hot inflow \textit{dominates} the accretion onto disk galaxies, rather than accretion by cool clouds condensed from the hot medium (Figs. 7 and C1, see also \citealt{Esmerian2021}).
...
We expect this dominance of hot inflows to be robust to resolution effects despite that the formation of cool clouds in simulations depends on resolution \citep[e.g.][]{McCourt2018, Mandelker2019a,Mandelker2021, Fielding2020}. This follows since the hot gas accretes on the cooling timescale (section 3.4), i.e., the same timescale on which thermal instabilities grow. Thermal instabilities in an inflowing hot CGM thus do not have time to grow substantially, as indicated by idealized calculations of cooling flows \citep{Balbus1989,Stern2019}.
...
The dark grey line in Fig.~\ref{f: before and after combined} shows the median properties of accreting gas in a simulation that includes cosmic ray physics~\citep[][]{Chan2019, Hopkins2020a}, thus providing a window into the differences in gas accretion modes in simulations with cosmic-ray (CR) support in the CGM.
...
Cool $\sim10^4$ K accreting gas can therefore act subsonic, and thus be conducive to thin disk formation, as long as $P_{\rm cr}$ is sufficiently large as is the case in CR-dominated haloes.
...
% (though Gandhi et al., in prep find that the SFR is consistent with other observational constraints).
...
% (although note that observational estimators do not fully capture SFR; \citealt{FloresVelazquez2021}).
...
% Spatial
Fig.~\ref{f: sample validation -- spatial} compares the properties of all gas in the CGM with the subset of gas which accretes onto the galaxy, corresponding to the main sample of tracked particles. % From Fig.~\ref{f: sample validation -- spatial} we learn
...
The haloes with $0.1 \lesssim \fthin < 0.6$ have varying levels of circularized cooling flow accretion.
