...
\begin{figure*}
    ...
    At $r\gtrsim0.1 R_{\rm vir}$ accreted gas either remains hot or cools only temporarily.
    \textbf{
...
\begin{figure}
    ...
    \caption{
    Angular distribution of accreting gas over 150 Myr before/after cooling for two MW-mass FIRE galaxies (\texttt{m12i\_md}, \texttt{m12b\_md}) and one LMC-mass central galaxy (\texttt{m11i\_md}).
    Angle is relative to the stellar disk, with $\cos\theta=0$ for gas along the disk plane.
    For $\sim L^*$ galaxies (top two panels) accreting gas is quasi-spherical prior to cooling while after cooling the gas has a disc-like configuration, indicating that cooling and circularization occur simultaneously. 
    In contrast, in the lower-mass halo in the bottom panel the gas is quasi-spherical both prior and after cooling, indicating that cooling is not associated with circularization.
    }
    \label{f: theta vs t}
\end{figure}
...
\textbf{Which term to use?}
Cooling flow accretion vs quiet accretion.
Or a different one?
Cooling flow accretion provides a stronger connection to previous work.
I use quiet accretion because cooling flow accretion is arguably a subset of a more general type of accretion.
I'd really like to use a single term, because people really catch on to that.
I'd also like the definition to be done with an eye towards the future.
