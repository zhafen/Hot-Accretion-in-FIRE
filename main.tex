...
(ii) this accretion cools to $\lesssim10^4\,{\rm K}$ just outside the galaxy, at $\approx4$ stellar half mass radii;
(iii) concurrent with cooling, the accreting gas \textit{circularizes}, i.e.\ the accretion flow transitions from a hot quasi-spherical configuration to a cool disk-like configuration. This circularization is preceded by the hot accretion particles exchanging angular momentum and developing a high-degree of coherent co-rotation, allowing accretion to quickly collapse into a thin gaseous disk when cooled to $T \approx 10^4$ K.
...
We show that the dominance of this ``rotating cooling flow'' accretion is strongly correlated with the fraction of thin disk stars central galaxies at $z\sim0$, across a sample of \Nsample~galaxies spanning a halo mass range of $10^{10.5} M_\odot \lesssim M_{\rm h} \lesssim 10^{12} M_\odot$.
...
Our results suggest that hot accretion may be a necessary condition for thin star-forming disks.
\end{abstract}
...
\textbf{Copy up top panels from Figure~\ref{f: before and after}.}
...
\begin{figure*}
    ...
    \caption{
    Mock Hubble images of four $z=0$ galaxies in the FIRE-2 simulations. Halo mass and the fraction of $<1$ Gyr old stars which reside in a thin disk are noted in the panels.  Milky-Way-mass galaxies in FIRE-2 typically have thin disk morphologies (top, bottom-left), while lower mass galaxies typically have irregular morphologies (bottom-right).
    \textbf{
...
The fact that thin disk galaxies are common only among fairly massive systems $L \sim 0.1 L_\star - L_\star$ at lower redshift~\citep[e.g.][]{Kranz2003, Kassin2006, Bizyaev2021}, is roughly consistent with the idea that cold-mode delivery is not conducive to thin disk formation.
% Hot mode
...
\hline
Name  &  $f_{\rm thin\,disk,\,young}$  & $V/\sigma$ & $R/h$ & $M_{\rm vir}$  &  $M_\star$  &  $R_{\textrm{vir}}$  &  $\Delta f_{\rm aligned}$  &  Physics  &  Reference  \\
  &   & & &  $M_\odot$  & $M_\odot$  &  kpc  &  &  &  \\
 \hline
...
$f_{\rm thin\,disk,\,young}$ is the fraction of stars formed in the last Gyr prior to $z=0$ that are in a thin disk configuration ($j/j_c(E) > 0.8$; e.g. \citealt{Yu2021}).
...
As noted in previous studies \cite{?} MW-mass galaxies in FIRE typically have thin disk morphologies, while lower mass galaxies show a thick disk / irregular morphology. We quantify this trend using $f_{\rm thin\,disk,\,young}$, defined as the fraction of stars with age $<1$ Gyr (at $z=0$) that have $j_z/j_c(E) > 0.8$, where $j_z$ is the specific angular momentum in the $z$ direction  and $j_c(E)$ is the angular momentum the star would have if it had the same energy but was in a circular orbit. The $z$ direction is defined as direction of the total angular momentum vector of stars inside the galaxy.  This definition of the thin disk is the same definition used in~\cite{Yu2021}. The values of $f_{\rm thin\,disk,\,young}$ for the four shown galaxies are noted in the panels. 
...
Temperature and spatial evolution of accreting gas with respect to $\tcon$, the time at which the gas last cools below $10^5$ K prior to accreting onto the galaxy. 
...
The panels show that accretion is hot ($\approx 10^6$) and contracts quasi-spherically at early times. Around the time of cooling, the geometry of accreting gas transitions from a quasi-spherical distribution at $t-\tcon < -150$ Myr to a cool ($\approx 10^4$ K) disk at $t-\tcon > 150$ Myr.
...
The top and right panels plot the temperature and spatial evolution of accreting gas versus $t-\tcon < 0$, while the bottom-left panel plots three representative trajectories for accreting gas elements. 
Red, white, and blue indicates $T=10^6$ K, $10^5$ K, and $10^4$ K respectively. The figure shows that the geometry of the accreting gas changes concurrently with cooling, transitioning from a hot, voluminous, pseudo-spherical distribution at $t-\tcon < -150$ Myr to a cool thin  disk at $t-\tcon > 150$ Myr. A quantitative analysis follows in sections \S\ref{s: characteristics -- inflowing gas phase}--\ref{s: ...}
...
The solid line and grey band show the distribution of individual accreted particles, while the dashed line shows the ratio for the net angular momentum of all accreted particles.
The shaded region shrinks as gas flows inward, indicating the flow becomes more coherent and that angular momentum unaligned with the total angular momentum is cancelled out as the accretion moves inwards.
\textbf{E:}
...
Density sharply increases as the gas cools, partially due to pileup, but the disk is nevertheless coherent well before being dense enough to form stars.
\textbf{G:}
Energy loss from radiative cooling (blue) and heating from $PdV$ work on the gas particles (red).
At $t<\tcon$ compression heating offsets radiative cooling, as expected in a cooling flow.
The energy balance falters as the gas obtains rotational support and piles up.
\textbf{Change radiation cooling to radiative cooling.}
}
...
\subsection{Accretion cools at the galaxy-halo interface}
...
% Rcon
Figure~\ref{f: R1e5K} shows the distribution of $\Rcon$ in the four galaxies, defined as
% A key quantity for differentiating modes of hot accretion is the radius at which gas last cools before accreting onto the galaxy, i.e. 
the particle radius at the time when it last cools, i.e., $\Rcon \equiv R (\tcon)$.
...
Gas with $\Rcon < R_{\rm gal}$ corresponds to gas which is under-dense relative to the galaxy ($n_{\rm H} < 0.13$ cm$^{-3}$) when it crosses $R_{\rm gal}$.
All particles with $\Rcon>80$ kpc are placed in the $R=80$ kpc bin.

Fig.~\ref{f: R1e5K} shows that in the three galaxies with a large thin disk fraction, the majority of the gas cools just outside the galaxy, with a median of $\Rcon \approx xx R_{\rm gal}\approx zz$ kpc and a $16-84$ percentile range of $yy-zz R_{\rm gal}$. Less than $10\%$ of particles cool beyond $\sim 40$ kpc.
This result is consistent with a hot cooling flow and quiet accretion, and inconsistent with a precipitation scenario where clouds cool far in the halo and subsequently accrete.

The lower-mass \texttt{m11q} has a similar median $\Rcon$, though with a wider distribution, than the more massive galaxies. Together with the result of Fig.~\ref{f: Mdot} that the inflow in this galaxy is mainly cool, this suggests that in this galaxy cool clouds are constantly reheated and cool down until they reach the galaxy. 
...
\begin{figure*}
    ...
    \caption{
    Angular distribution of accreting gas versus time relative to last cooling time. 
    Gas along the disk plane has $\cos\theta = z/R = 0$, while a spherical distribution would have a PDF of $0.5$ at all $z/R$. 
    The angular distribution of gas accreted onto the thin disk galaxies evolves significantly at the time of cooling (top and bottom-left panels), from a quasi-spherical distribution prior to cooling to a disk-like configuration after cooling. 
    In contrast, in the lower-mass halo the gas is roughly spherical both prior to and after cooling.
...
The curves show the PDF of $\cos \theta = z/R$, where $\theta$ is defined as the angle between the gas element position and the total angular momentum vector of the stars inside the galaxy.
...
The plot shows that gas accreting onto the three thin disk galaxies transitions from being distributed quasi-spherically at $t = \tcon-200$ Myr to being distributed in the galaxy plane at $t = \tcon+200$ Myr.
This indicates that cooling and circularization of the accreted gas occurs simultaneously in these galaxies. In contrast, in the irregular galaxy there is no association between cooling and circularization, and the geometry of accreting gas is quasi-spherical both before and after cooling. 
...
\begin{figure*}
    ...
    \caption{
    \textbf{Left:}
    Mass fraction of accreting gas aligned with the disk ($z/R < 0.1$) before and after cooling (red and blue respectively) for our sample of \Nsample~analyzed halos. The horizontal axis plots the fraction of stars formed within the last Gyr prior to $z=0$ that are in a thin disk.
    ...
    \textbf{Right:}
    Change in aligned mass fraction from 200 Myr before to 200 Myr after cooling time. Color indicates virial mass. The grey line has a slope of unity. The change in 
    This quantity scales with the fraction of mass accreted as a rotating cooling flow.
    $\Delta f_{\rm aligned}$ strongly tracks thin disk fraction, and \textit{every galaxy that forms more than half its stars in a thin disk is fueled primarily by rotating cooling flows}.
    \textbf{Circle and talk about the outlier.}
    \textbf{Make legend alpha more opaque.}
...
Figure~\ref{f: prevalence} extends the analysis in Fig.~\ref{f: theta vs t} to the full. We parametrize the extent to which circularization and cooling are concurrent via a parameter $f_{\rm aligned}$, which corresponds to the fraction of accreting gas mass aligned with the disk plane ($\vert z/R \vert < 0.1$, dashed verticl lines in Fig.~\ref{f: theta vs t}. This range contains of angles contains $\approx 60\%$ of stars in thin disk galaxies.
The left panel of Fig.~\ref{f: prevalence} shows the evolution of $f_{\rm aligned}$ from from 200 Myr before $\tcon$ (red) to 200 Myr after $\tcon$ (blue), while the right panel shows the difference between these two epochs. 
...
In halos with $f_{\rm thin\,disk} > 0.5$ the alignment of the accreted gas sharply increases at $\tcon-200\,{\rm Myr}<t<\tcon+200 Myr$ --- in most cases $\gtrsim 50\%$ of mass collapses to $\vert z/R \vert < 0.1$.during this time. 
In contrast, in halos with $f_{\rm thin\,disk} \approx 0$ the change in $f_{\rm aligned}$ is smaller, if any. % Change in aligned mass fraction
The right panel shows there is a strong relation between the change in $f_{\rm aligned}$ when the gas cools and fraction of young stars in a thin disk. 
% To track the trend we draw a simple line between $(f_{\rm thin\,disk}, \Delta f_{\rm aligned}) = (0,0)$ and the average $(f_{\rm thin\,disk}, \Delta f_{\rm aligned})$ for galaxies with $f_{\rm thin\,disk} > 0.5$.
% Halos with $f_{\rm thin\,disk} > 0.5$ have $\Delta f_{\rm aligned} > 0.2$ and halos with $f_{\rm thin\,disk} < 0.5$ have $\Delta f_{\rm aligned} < 0.2$, including four halos with $0.1 < f_{\rm thin\,disk} < 0.5$.
Fig.~\ref{f: prevalence} thus demonstrates on the entire sample that circularization is concurrent with cooling in galaxies with large $f_{\rm thin\,disk$, while no such association exists for galaxies with small $f_{\rm thin\,disk}$.



% Presence of a rotating cooling flow for intermediate cases
The halos with $0.1 < f_{\rm thin\,disk} < 0.5$ have varying levels of circularized cooling flow accretion.
...
\begin{figure*}
    ...
    \caption{
    Evolution of angular momentum coherence, shown by the evolution of the fraction of mass above a given $j_z / \vert \vec j \vert$ over 1.5 Gyr around $\tcon$.
    Gas with angular momentum aligned with the stellar angular momentum (i.e. fully aligned) has $j_z / \vert \vec j \vert$ = 1.
    ...
    \textbf{Make sure the captions aren't being cut off by their outline.}
    }
    \label{f: coherence}
\end{figure*}
...
\subsection{Hot, inflowing, extended to cooled, stalled, collapsed}
...
Panels (A), (B), and (C) in Figure~\ref{f: before and after} respectively show gas temperature, radius, and absolute height at a given time relative to the final cooling time, $\tcon$, in one of thin disk galaxies. 
...
The radius panel demonstrates that last cooling occurs at $R_{\rm gal}$, consistent with Figure~\ref{f: R1e5K}.
...
Panel (C) shows that at the same time the gas collapses to a median height of $\lesssim 1$ kpc.
Note that at this the gas is very close to the center of the disk plane, it is still at the outer edge of the galaxy, $R \sim R_{\rm gal}$.

\subsection{Angular momentum support and coherence}
...
\subsection{Support leads to cooling}
