...
Among central galaxies with a high fraction ($>70\%$) of recent stars in a thin disk ($h/R \sim 0.1$) we find that:
...
We find that thin disk galaxies in FIRE accrete via `rotating cooling flows', a mode of `hot accretion' in which the quasi-spherical $T\sim\Tvir$ CGM phase inflows towards the galaxy, remaining hot down to the radius where its angular momentum is sufficient to provide rotational support. At this radius ($\gtrsim 4 R_{1/2}$, just outside the galaxy radius) the hot inflow both decelerates and becomes coherently rotating, and then simultaneously cools and collapses into a rotating cool disk. Our results thus extend classic cooling flow theory by demonstrating their applicability in realistic cosmological simulations, and by exploring the mechanics of cooling flows with angular momentum, a subject which has not yet been studied extensively \citep[c.f.][]{Cowie1980,Stern2020a}. Moreover, we find a strong correlation between the prevalence of this accretion mode and the fraction of stars in the central galaxies that form in a thin disk, potentially indicating that the existence of rotating cooling flows is a necessary condition for the formation of thin star-forming disks.
...
Two properties of rotating cooling flows likely promote the formation of thin star forming disks in the central galaxy -- their tendency to become coherently rotating (Fig.~\ref{f: coherence} and panels A--B in Fig.~\ref{f: before and after B}) and their tendency to decelerate before accretion (panel D in Fig.~\ref{f: before and after A}). Combined, these two properties indicate that upon cooling the accretion flow forms a thin disk with $v_\phi\approx v_{\rm c}$ and $v_r, v_\theta \ll v_{\rm c}$. 
Though any accreting collisional medium is expected to eventually equilibriate into such a coherently rotating disk, in rotating cooling flows such disks form \textit{prior} to cooling and accretion onto the ISM. Thus, when accretion occurs via rotating cooling flows star formation commences only after a thin gaseous disk morphology is achieved (Fig.~\ref{f: before and after A}), while in other accretion modes stars may start to form prior to the accretion settling in a thin disk. Also, this smooth accretion onto the disk implies that rotating cooling flows do not inject a large amount of momentum and energy into the ISM, but rather radiate their gravitational energy away prior to accretion. This is in contrast with cold flow accretion which potentially halts via a shock, thus stirring turbulence in the ISM and making the gaseous and stellar disk thicker (\citealt{Dekel2009a}). 
...
We attribute these properties to the subsonic nature of the flow. 
Deceleration is expected in a subsonic flow since the pressure structure on CGM scales can adjust to the transition to rotational support at the galaxy scale, thus allowing the radial velocity to gradually decrease to zero. This is in contrast with cold flow accretion which is supersonic, and thus expected to halt at the galaxy scale via a shock. 
% consistent with the behavior of 1D steady-state solutions of subsonic cooling flows with a centrifugal term \citep{Cowie1980, Stern2020a}. 
Also, angular momentum coherence is expected since $v_{\rm c}^2\approx c_{\rm s}^2$ in hot inflows where $c_{\rm s}$ is the sound speed, and hence at the rotational support radius where the mean $\langle v_\phi\rangle\approx v_{\rm c}$ (by definition) the mean Mach number in the $\phi$ direction $\langle v_\phi\rangle/c_{\rm s}$ approaches unity. Any velocity fluctuations around this mean would hence lead to supersonic speeds that quickly shock and convert their kinetic energy into thermal energy. We thus expect any velocity fluctuations induced by a broad distribution of angular momentum to rapidly die out when the hot flow approaches the rotational support radius, yielding a coherent accretion flow. 
...
it has rotating cooling flow accretion comparable to thin disk galaxies ($\Delta f_{\rm aligned} \approx 0.25$), but has a relatively small $f_{\rm thin\,disk,\,recent} \approx 0.4$ (yellow square in Fig.~\ref{f: prevalence}). However, the strong correlation seen in Fig.~\ref{f: prevalence} between the prevalence or rotating cooling flow accretion (quantified via $\Delta f_{\rm aligned}$) and the thin disk fraction $f_{\rm thin\,disk,\,recent}$ suggests that such events are relatively rare. 
    

% A number of previous studies have noted that angular momentum alignment in the CGM is crucial for disk formation, consistent with the claim in this paper. \cite{Sales2012} find in their cosmological simulations that angular momentum alignment of accreting gas is better correlated with disk formation than any dark matter halo property. \cite{Kretschmer2020} similarly point to the alignment of angular momentum as the distinguishing factor between a cosmological zoom-in of a disk galaxy and a zoom-in of a bulge-dominated galaxy with similar mass accretion histories and halo spin parameters. Rotating cooling flows provides an explanation why alignment contributes to disk formation. 
...
The interpretation in this paper that thin star forming disks are a result of accretion via rotating cooling flows, allows us to predict at which halo masses and redshifts we expect thin disks to form. A prerequisite for accretion via cooling flows is that the cooling time of hot shocked gas $\tcoolsh$ in the CGM exceeds the free-fall time $\tff$,  causing a time-steady and pressure-supported `virialized CGM' to form \citep{White78, White1991}. Even in the presence of such a virialized CGM, cooling flows may be subdominant to cold flows that penetrate the hot halo, especially at $z>1$ \cite[e.g.,][]{Keres2005,Dekel2006, Dekel2009}, or could be disrupted by heating from feedback as envisioned in `thermal-balance' CGM models \citep[e.g.,][]{McCourt2012, Sharma2012, Voit2017, Faerman2017, Faerman2019}.
Furthermore, \cite{Stern2020a} demonstrated that even if a virialized CGM exists at large halo scales the resulting cooling flow may go through a sonic point and reach the galaxy as cool supersonic accretion. A cooling flow which remains hot and subsonic down to the galaxy scale, as found above for thin disk galaxies in FIRE, is thus only possible when the inner CGM is also virialized, which occurs when $\tcoolsh$ exceeds $\tff$ at $\approx0.1 r_{\rm vir}$. \cite{Stern2021} showed that this condition is satisfied in FIRE at $\Mvir\approx10^{12}\msun$, roughly independent of redshift. 

Figure~\ref{f: prevalence vs galaxy properties} shows the relationship between thin disk fraction and virial mass (left panel), stellar mass (middle), and $\tcoolsh/\tff$ at $0.1 r_{\rm vir}$ in our sample of $z\sim0$ FIRE galaxies. Markers are colored by $\Delta f_{\rm aligned}$, our measure of the dominance of rotating cooling flows in accreting gas (Fig.~\ref{f: prevalence}). Indeed, thin disks appear and rotating cooling flows appear only at $M_{\rm vir}\approx 10^{12}\msun$ and $\tcoolsh/\tff(0.1\Rvir)>1$, consistent with the expectation that rotating cooling flows and thin disks commence when the inner CGM virializes. This result also indicates that any heating of the CGM by stellar feedback at $z\sim0$ is insufficient to disrupt the cooling flow, consistent with the weak outflows at this mass and redshift scale both in FIRE \cite{Muratov2015, Muratov2017, Stern2021, Pandya2021} and in observations \cite{HeckmanThomspon}. The potential effects of AGN feedback, not implemented in the FIRE sample used in this work, are discussed in section xx. 


Additional properties of the ISM change concurrently, namely 
...
\cite{Stern2021} showed that at higher redshift of $z\sim2$, the formation of disks are also associated with the virialization of the inner CGM, though these disks tend to be thicker with $V_{\rm rot}/\sigma_z\approx2-3$ in contrast with $V_{\rm rot}/\sigma_z\approx7-10$ in the low-redshift disks discussed above (see figs. 13-14 there), consistent with the thicker high-redshift disks seen in observations \citep{}. Potentially, the thicker disks at high redshift are a result of cold flow accretion dominating over cooling flows at this epoch. We will explore this possibility in FIRE in future work. 
To summarizes, the picture which emerges is that `disk settling', i.e.~the transition from irregular morphology and dispersion-dominated kinematics to disky morphology and rotation dominated kinematics is a result of inner CGM virialization, while the formation of \textit{thin} disks with $V_{\rm rot}/\sigma_z\approx7-10$ further requires that accretion is dominated by rotating cooling flows. 
...
\subsubsection{Star formation rates fueled by rotating cooling flows}
\label{s: fueling}
