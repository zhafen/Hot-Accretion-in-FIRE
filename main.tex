...
\begin{figure*}
    ...
    \textbf{What's with the values above 1 or below 0?}
...
% CHARACTERIZATION OF ACCRETING GAS - TRACKS and R1e5
\begin{figure*}
    \centering
...
% CHARACTERIZATION OF ACCRETING GAS - ANGULAR DISTRIBUTION
\begin{figure}
    \centering
...
% COOLING AND ACCRETION MECHANISMS
\begin{figure*}
% \includegraphics[width=\columnwidth]{rad_vs_compress.png}
\includegraphics[width=\textwidth]{figures/rad_vs_compress.pdf}
\caption{
{\bf JB:   As an alternative to the second panel, it would be interesting to plot the dimensionless ratio of $j_z/(r V(r))$ — a direct measure of circular support.}
\textbf{Top:}
...
Really do look at the ratio of angular momentum to rVc (don't forget!)
}
}
...
% Our simulations and their generality
\textbf{While SNe feedback is crucial, it shouldn't affect the CGM at this redshift.}

% This paper
\textit{
In this paper we assess the mechanism through which gas accretes onto central Milky Way-mass halos at $z=0$ in a sample of modern cosmological simulations.
In \S\ref{s: inflowing gas phase} we assess the relative contribution between cold and hot mode accretion, revealing that hot mode accretion dominates.
In \S\ref{s: accreting gas characteristics} we characterize the accreting gas, including its temperature and spatial history, and show that gas accretes directly into or around the disk of the central galaxy.
In \S\ref{s: accretion mechanisms} we analyze the physical mechanisms responsible for disk-accreting hot mode.
In \S\ref{s: discussion} we discuss the implications of our results, including the conditions under which accretion should be dominated by disk-accreting hot mode (\S\ref{s: generality}) and a comparison to other accretion modes (\S\ref{s: other modes}).
...
\subsection{Characteristics of accreting gas}
\label{s: accreting gas characteristics}
...
\subsubsection{Accreting gas cools at or near the galaxy edge}
...
\subsubsection{Accreting gas forms a disk as it cools}

% Figure description/introduction, and more

\subsection{Cooling and accretion mechanisms}
\label{s: accretion mechanisms}
...
\subsubsection{Extension to high redshift}

\subsubsection{Extension to systems with nonthermal support}

\subsection{Contrast with other accretion modes}
\label{s: other modes}
...
\subsection{Connection to SFR}

\textbf{Discrepancy between the SFR the FIRE sims have and the SFR the cooling flow models assume and which works?}

\textbf{Read Viraj's recent paper.}
...
\section{Metal diffusion vs non-metal diffusion vs MHDCV vs CRs}
