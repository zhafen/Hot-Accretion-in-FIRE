...
The fact that thin disk galaxies are common only among fairly massive systems ($L \sim 0.1 L_\star - L_\star$) at lower redshift~\citep[e.g.][]{Kranz2003, Kassin2006, Bizyaev2021, Kassin2012a, Simons2015, Simons2017}, suggests that cold-mode delivery may not be conducive to thin disk formation.
...
In cold-mode halos such heating is typically temporary, but still comprises $\gtrsim 30\%$ of the halo's gas mass at any given time~\cite{Hafen2019,Hafen2020}.
...
A quantitative analysis of this transition follows in \S\ref{s: characteristics -- inflowing gas phase}--\ref{s: mechanics -- energy balance}.
...
Panel (C) in Fig.~\ref{f: before and after A} demonstrates that cooling at the galaxy scale is further associated with a change in accretion kinematics (with z-axis oriented along the total angular momentum of stars in the galaxy at $z=0$).
...
Similar results are seen for accretion onto other thin-disk galaxies in our sample (appendix \ref{s: appendix-more before and after}). 
Specifically, \textit{the majority of accretion onto $z\sim0$ thin disk galaxies in FIRE is a hot inflow which cools and decelerates just outside the galaxy}.
...
We argue here that these characteristics are a result of the subsonic nature of the flow. 
...
The scenario suggested by this paper, \textit{that thin star forming disks are a result of accretion via rotating cooling flows}, allows us to predict at which halo masses and redshifts we expect thin disks to form.
Our predictions are based on the mechanics of rotating cooling flows, as gleaned from our analysis focused on $z\sim 0$.
...
That stars in irregular FIRE galaxies are formed on primarily eccentric, unaligned orbits~(\citealt{Yu2021}, Yu et al., in prep) indicates that ISM processes cannot create narrow, closely-aligned angular momentum distributions in some galaxies.
...
(C) the onset of rotating cooling flows is coincident with, but independent of, the presence of ISM processes that increase alignment.
...
At $z\sim0$, FIRE galaxies only have significant rotational support ($V_{\rm rot}/\sigma_z \gg 1$) above $M_\star\sim10^{10}\msun$, in contrast with observations which find rotationally-supported galaxies above $M_\star\sim10^9\msun$ (\citealt{El-Badry2018a, El-Badry2018}, see also \citealt{Peebles2020}).
