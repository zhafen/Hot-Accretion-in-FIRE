...
The analysis here follows \cite{Stern2021}, which showed that the formation of discs in FIRE is closely connected to the formation of a virialized and stable hot CGM.
\cite{Yu2021} subsequently showed that the emergence of an inner virialized CGM in FIRE simulations correlates specifically with a transition from {\em thick}-disc to {\em thin}-disc formation.
...
These simulations include a combination of cosmological environment and galactic physics, which has allowed simulators to investigate accretion while accounting for both its cosmological nature and its interaction with galaxies and their CGM~\citep[e.g.][]{Oppenheimer2010, Stewart2011, Fernandez2012, Ford2014, Wetzel2015, Angles-Alcazar2017, Hafen2019, Hafen2020, Ho2019, Rottgers2020, Trapp2021}.
Of special note are ``zoom-in'' simulations that focus on resolving a single halo and its environment, enabling high resolution while preserving cosmological context~\citep[e.g.][]{Katz1993, Onorbe14, Wang2015, Agertz2021}.
Among these, the FIRE simulations~(Feedback in Realistic Environments; \citealt{Hopkins2014, Hopkins2018})\footnote{\url{https://fire.northwestern.edu/}} are a set of zoom-in simulations that resolve stellar feedback on the scale of giant molecular clouds in the interstellar medium (ISM)~\citep{Guszejnov2020b, Benincasa2020}, producing winds that naturally expand into the CGM and interact with accreting gas~\citep{Muratov2015, Muratov2017, Angles-Alcazar2017, Hafen2019, Hafen2020, Pandya2021}.
...
\begin{figure}
    ...
    The properties of our main sample are representative of all recent star formation in regards to the epoch at which gas accretes onto the galaxy, and overlaps with the accretion times at which \textit{all} thin disk formation peaks.
    }
...
\begin{figure}
    ...
    \caption{
    \textbf{Topmost panel:}
    Radial distribution for the main sample vs for all gas.
    The main sample preferentially probes the inner CGM, as that is what can accrete prior to $z=0$.
    \textbf{Bottom panels:}
    ...
    Opacity (darkness) of the accreted gas distribution scales with amount of the sample found at that radius.
...
Our sample was selected to allow us to track a Lagrangian region with high resolution.
Sample selection details are in \S\ref{s: methods -- analysis}.
In this appendix we check the extent to which our sample is representative of all gas in the halo at the same redshift, accretion that fuels all recent star formation, and all thin disk stars.
\textbf{Describe samples in detail, including sampling.}

% Accretion times
The degree of overlap we see in the $t_{\rm acc}$ distributions between our sample and all recent star formation validates the stronge correlation between properties of our sample and the fraction of recently formed stars found in a thin disk (Figs~\ref{f: prevalence} and~\ref{f: prevalence - angular momentum}).
For our main sample we choose the time of first accretion after being located in the CGM.
For the other two samples we choose the time of last accretion.
\textbf{Number of particles.}

% Spatial
\textbf{Link to CGM fates paper.}

% Takeaway
\textbf{Our sample is largely representative.}
\textbf{By tracking all gas accretion at a given time we can do things like energy balance.}
...
\textbf{G:} The magnitude of the specific angular momentum of particles ($\mid\vec{j}\mid$, green) and the component of angular momentum aligned with the galaxy disc ($j_z$; purple).
The dashed line shows the angular momentum necessary for rotational support.
