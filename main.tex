...
Note that the \texttt{m12m} galaxy (marked by $^*$) has a sizable bar, which forms secularly at late times and drives the thin-disk fraction lower by our adopted definition~\citep{Debattista2019}.
...
At $t = \tcools$ the gas cools (by definition), and achieves cool ISM temperatures or forms stars at later times (the vertical grey line marks the median accretion time). 
\textbf{B:}
...
\begin{figure*}
    ...
    \caption{
    Average properties relative to time of accretion for thin disk galaxies (left; $f_{\rm thin\,disk}>0.5$) and irregular galaxies (right; $f_{\rm thin\,disk}<0.5$).
    ...
    The solid black lines in the left/right columns show the median values for a representative thin disk/irregular galaxy (\texttt{m12i}/\texttt{m11d}).
    The dashed line shows the median values for a cosmic ray-dominated halo, discussed in \S\ref{s: crs}.
    From top to bottom the properties shown are temperature, radial distance, ratio of absolute height to radius, ratio of angular momentum magnitude to circular angular momentum, and fraction with $j_z / \vert j \vert > 0.9$ (i.e. fraction with an aligned angular momentum).
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figures/variations/relative_to_accretion/prevalence/delta_smooth_disk_frac_v_thin_disk_frac_recent.pdf}
    \caption{
    Angular momentum-space equivalent of Fig.~\ref{f: prevalence}.
    \textbf{Left:}
    Mass fraction of accreting gas aligned with the disk before and after cooling, defined in this figure as the fraction with $\vert j_z/\vert j \vert > 0.9$ (see Fig.~\ref{f: coherence}).
    ...
    Change in aligned mass fraction during the $\pm200$ Myr around cooling time shown in the left panel.
    The greater the extent to which accretion becomes coherent as it cools, the greater the fraction of young stars in a thin disk.
    \textbf{Add note: prior to accretion, and time label.}
    \textbf{Remove left panel.}
    \textbf{Try post dt = 0.}
    }
    \label{f: prevalence - angular momentum}
\end{figure}
