...
Kareem El-Badry$^{11,12}$,
T. K. Chan$^{13}$,
Cameron Trapp$^{14}$,
Philip F. Hopkins$^{10}$,
Du\v{s}an Kere\v{s}$^{14}$
} \vspace{0.4cm}\\
\parbox{\textwidth}{
...
$^{12}$ Harvard Society of Fellows, 78 Mount Auburn Street, Cambridge, MA 02138 \\
$^{13}$ Institute for Computational Cosmology, Department of Physics, Durham University, South Road, Durham DH1 3LE, UK \\
$^{14}$ Department of Physics, Center for Astrophysics and Space Sciences, University of California, San Diego, 9500 Gilman Drive, \\ La Jolla, CA 9209, USA \\
}
}
...
(iii) prior to cooling, the accreting gas develops an angular momentum distribution that is aligned with the galaxy disk, and transitions from a quasi-spherical spatial configuration to a more flattened, disk-like configuration.
...
\textbf{Try making these a bit taller, and labels a bit bigger.}
}
...
As shown below (\S\S\ref{s: thin vs irregular}), this is {\em not} the case for galaxies that lack thin disks in our sample. 
...
\begin{figure*}
    ...
    \caption{
    Geometry (ratio of height to radius) of accreting gas, as a function of time relative to the last cooling time and for the same galaxies as in Fig.~\ref{f: stars}.
...
\begin{figure*}
    ...
    \textbf{Mention or remove the line.}
    }
    \label{f: prevalence}
\end{figure*}
...
% As To avoid optically thick gas We focus on gas with $>10^4.5$
...
\begin{figure*}
    ...
    The dark grey lines show the median values for a cosmic ray-dominated halo, discussed in \S\ref{s: crs}.
...
% Also in thin disk galaxies $\tacc$ differs from $\tcools$ if the angular momentum support radius where the gas cools differs from $r_{\rm gal}$
...
% An exception is when the angular momentum support radius is larger than the current galaxy radius ($\vert j \vert > r_{\rm gal} v_c( r_{\rm gal})$), and therefore hot accretion will cool outside the galaxy and possibly attain ISM densities prior to reaching $r_{\rm gal}$.
...
% This is especially true for galaxies fed primarily by cold streams, because gas is typically only hot for a short period of time in such halos, and for halos with $\vert j \vert > r_{\rm gal} v_c( r_{\rm gal})$.
% One halo with larger-than-usual median $\vert j \vert$ is \texttt{m12f}, which has similar behavior as the other thin disk galaxies relative to $t - \tcools$ but unusual behavior relative to $t - \tacc$.
% We show the properties of  as a function of time in Appendix~, and exclude its contribution to the medians in Figure~\ref{f: before and after combined}..
...
\begin{figure*}
    ...
    \caption{
    Angular momentum distribution of accreting gas, as a function of time relative to accretion time and for the same galaxies as in Figs.~\ref{f: stars},~\ref{f: theta vs t}.
    ...
    \textbf{Consider making co-rotating arrow larger and clearly an arrow.}
    }
    \label{f: coherence}
\end{figure*}
...
% The average gas element has sufficient time to equilibrate spends ${\sim} 0.5-2$ Gyr in the ISM prior to star formation , substantially longer than the dynamical timescale of $\sim100$ Myr on which coherence can be achieved, our results potentially indicate that accretion via rotating cooling flows creates disk which are resilient to such disruption. Why would that be the case?
...
% would create fluctuations in the angular momentum distribution with a relative magnitude of $\approx 1/3$ at $r=\Rcirc$, while it will create stronger, order-unity fluctuations at a larger radius of $r=3\Rcirc$. This follows since , implying a turbulent velocity  
...
% At the same time, \cite{Trapp2021} find rough agreement with galaxy-halo interface cool gas observations in the fraction and amount of mass accreted as HVCs and IVCs, and in the radial velocities at the disk edge.
% Yet above we note that a lower CGM mass may help produce SFRs and rotationally-supported galaxy populations more consistent with observations, and a higher CGM mass would alleviate a potential discrepancy with X-ray emission.
